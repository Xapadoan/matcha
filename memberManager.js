	...
	module.exports = {
		...
		getUserImages: function getUserImages(username) {
			return (new Promise ((resolve, reject) => {
				connection.query('SELECT u.id, image1, image2, image3, image4, image5 FROM matcha.users_images INNER JOIN matcha.users u ON matcha.users.id = matcha.users_images.user AND matcha.users.username = ?',[
					username
				], (err, results) => {
					if (err) {
						console.log(err.stack);
