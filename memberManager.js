	...
	delete_user: function delete_user(username, password) {
        return (new Promise((resolve, reject) => {
            if (username && password) {
                connection.query('DELETE matcha.u, matcha.users_extended, matcha.users_images, matcha.users_interests, matcha.users_likes, matcha.users_dislikes, matcha.users_blocks, matcha.users_reports, matcha.users_visits FROM matcha.users u INNER JOIN matcha.users_extended e ON u.id = e.user INNER JOIN matcha.users_images i ON u.id = i.user INNER JOIN matcha.users_interests n ON u.id = n.user INNER JOIN matcha.users_likes l ON (u.id = l.liked OR u.id = l.liker) INNER JOIN matcha.users_dislikes d ON (u.id = d.disliker OR u.id = d.disliked) INNER JOIN matcha.users_blocks b ON (u.id = b.blocker OR u.id = b.blocked) INNER JOIN matcha.users_reports r ON u.id = r.reported INNER JOIN matcha.users_visits v ON (u.id = v.visitor OR u.id = v.visited) WHERE username = ?', [username], function (error, results) {
                    if (error) {
